  ...
  const fetchDirections = async () => {
    ...
    try {
      ...
      if (modeRecherche && search.trim() !== "") {
        ...
        res = await axios.get(`${API_BASE_URL}/directions?page=${pagination.current_page}&per_page=${pagination.per_page}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1.5 text-sm text-slate-500">Gérez les directions de votre organisation en toute simplicité</p>
              </div>
              <nav className="flex items-center gap-1.5 text-sm">
                <a href="/" className="text-slate-500 hover:text-indigo-600 transition">Accueil</a>
                <span className="text-slate-300">/</span>
                <span className="font-medium text-slate-900">Directions</span>
              </nav>
            </div>
          </div>
          ...
          <div className="mb-6 flex flex-col gap-4 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/60 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-1 items-center gap-2">
              ...
              <button
                onClick={handleSearch}
                className="sm:hidden inline-flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-600 text-white shadow-sm"
              >
                <FaSearch />
              </button>
              <button
            ...
            <button
              onClick={() => { setDirectionToEdit(null); setShowModal(true); }}
            ...
            >
              <FaPlus className="text-xs" /> Ajouter une Direction
            </button>
          </div>
          ...
          {loading ? (
            <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200/60">
              <LoadingSpinner
                message="Chargement des directions..."
                    ...
                    <tbody className="divide-y divide-slate-100 bg-white">
                      {directions.length === 0 ? (
                          ...
                          <td colSpan={5} className="px-6 py-16 text-center">
                            <div className="mx-auto max-w-sm">
                              ...
                              <p className="mt-1 text-sm text-slate-500">
                                {modeRecherche ? "Aucun résultat pour votre recherche" : "Commencez par ajouter une direction"}
                              </p>
                            </div>
                          </td>
                        </tr>
                        ...
                        directions.map((direction, index) => (
                          <tr key={direction.id} className="hover:bg-slate-50/70 transition">
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-semibold text-slate-900">{direction.nom}</span>
                              </div>
                            </td>
                            <td className="whitespace-nowrap px-6 py-4 text-center">{getStatusBadge(direction.statut)}</td>
                            ...
                            <td className="whitespace-nowrap px-6 py-4 text-center">
                              <ActionDropdown>
                                <button onClick={() => handleEdit(direction)} className="flex w-full items-center gap-3 px-3 py-2.5 text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 transition">
                                ...
                                <button onClick={() => handleDelete(direction.id)} className="flex w-full items-center gap-3 px-3 py-2.5 text-sm text-rose-600 hover:bg-rose-50 transition">
              ...
              {directions.length > 0 && (
                <div className="mt-4 flex flex-col gap-4 rounded-2xl bg-white px-5 py-4 shadow-sm ring-1 ring-slate-200/60 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <p className="text-sm text-slate-500">
                    Affichage de <span className="font-semibold text-slate-900">{(pagination.current_page - 1) * (pagination.per_page || 10) + 1}</span> à{" "}
                    <span className="font-semibold text-slate-900">{Math.min(pagination.current_page * (pagination.per_page || 10), pagination.total || directions.length)}</span> sur{" "}
                    <span className="font-semibold text-slate-900">{pagination.total || directions.length}</span> directions
                  </p>
                  <select
      ...
      <ModalDirectionScreen
        isOpen={showModal}
        onClose={() => { setShowModal(false); setDirectionToEdit(null); }}
