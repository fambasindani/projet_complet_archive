...
const ClasseurScreen = () => {
    ...
    const [statsCards, setStatsCards] = useState([
        {
            ...
            value: "0",
            icon: <FaFolder className="text-white text-xl" />,
    ...
    const fetchClasseurs = async () => {
        ...
        try {
            ...
            if (modeRecherche && search.trim() !== "") {
                ...
                res = await axios.get(`${API_BASE_URL}/classeurs?page=${pagination.current_page}&per_page=${pagination.per_page}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            ...
            console.error(err);
            toast.error("Erreur lors du chargement des classeurs");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = (id) => {
        if (!token) { toast.error("Vous n'Ãªtes pas authentifié. Veuillez vous reconnecter."); return; }
        const permissions = JSON.parse(localStorage.getItem("permissions") || "[]");
        if (!permissions.includes("supprimer_classeur")) { toast.error("Vous n'avez pas la permission 'supprimer_classeur' pour désactiver cet élément."); return; }
        const _item = classeurs.find((x) => x.id === id);
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axios.delete(`${API_BASE_URL}/classeurs/${confirmItem.id}`, { headers: { Authorization: `Bearer ${token}` } });
            ...
            if (error.response) {
                ...
                else if (status === 404) errorMessage = data?.message || "Ã‰lément non trouvé.";
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="mb-8">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-2xl font-bold tracking-tight text-slate-900">Gestion des Classeurs</h1>
                                    <p className="mt-1 text-sm text-slate-500">Gérez les classeurs de votre organisation</p>
                                </div>
                            </div>
                            <nav className="flex items-center gap-2 text-sm">
                                ...
                                <span className="font-medium text-slate-900">Classeurs</span>
                            </nav>
                        </div>
                    </div>
                    ...
                    <div className="mb-6 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/60">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            ...
                            >
                                <FaPlus className="text-xs" /> Ajouter un Classeur
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60">
                        {loading ? (
                            <div className="py-8">
                                <LoadingSpinner message="Chargement des classeurs..." variant="table" size="lg" />
                            </div>
                        ) : classeurs.length === 0 ? (
                            <div className="py-16 text-center">
                                ...
                                <h3 className="mt-4 text-sm font-semibold text-slate-900">Aucun classeur trouvé</h3>
                                <p className="mt-1 text-sm text-slate-500">
                                    {modeRecherche ? "Aucun résultat pour votre recherche" : "Commencez par ajouter un classeur"}
                                </p>
                                {!modeRecherche && (
                                    <button
                                        onClick={() => {
                                ...
                                <table className="w-full divide-y divide-slate-200">
                                    ...
                                    <tbody className="divide-y divide-slate-100 bg-white">
                                        {classeurs.map((classeur, index) => (
                                            <tr key={classeur.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="text-sm font-semibold text-slate-900">{classeur.nom_classeur}</span>
                                                    </div>
                                                </td>
                                                <td className="whitespace-nowrap px-6 py-4 text-center">{getStatusBadge(classeur.statut)}</td>
                                                ...
                                                <td className="whitespace-nowrap px-6 py-4 text-center">
                                                    <ActionDropdown>
                                                        ...
                                                        <button
                                                            onClick={() => setDetailItem(classeur)}
                                                        ...
                                                        <button
                                                            onClick={() => handleDelete(classeur.id)}
                        ...
                        <div className="mt-4 flex flex-col gap-4 rounded-2xl bg-white px-5 py-4 shadow-sm ring-1 ring-slate-200/60 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-center gap-3">
                                <p className="text-sm text-slate-500">
                                    ...
                                    <span className="font-semibold text-slate-900">{pagination.total}</span> classeurs
                                </p>
                                <select
